...
norm_xcc = 'u'; %switch to 'u' for un-normalized xcc - faster but less accurate
...
% %     z = size(I0{1});
% %     imgPadded = zeros([sz(1:2)+2*prePad size(I0{1},3)]);
% %     imgPadded((1:sz(1))+prePad(1), (1:sz(2))+prePad(2), :) = I0{1};
% %     I{1} = imgPadded;


%     z = size(I0{i});
%     imgPadded = zeros([sz(1:2)+2*prePad size(I0{i},3)]);
%     imgPadded((1:sz(1))+prePad(1), (1:sz(2))+prePad(2), :) = I0{i};
%     I0{i} = imgPadded;
...
x_high = x(x>muStart(1));
x_center = x_high(x_high<muStart(2));

single_distro = 1;
paramEsts = [0 0 median(x_center) 0 std(x_center)];
